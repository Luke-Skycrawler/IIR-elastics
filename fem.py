...
class QuadFEM: 
        ...
        for e in range(n_elements):
            for iq in range(nq):
                ...
                for _i in range(8):
                    ...
                    bi, dbidx = QuadFEM.bf_trilinear(i, x)
                    ...
                    for _j in range(8):
                        j = self.nodes[e, _j]
                        bj, dbjdx = QuadFEM.bf_trilinear(j, x)
                        for k in ti.static(range(3)):
                            # fill in by row
    ...
    def xq(e, I):
        ...
        x0 = QuadFEM.xc(QuadFEM._trans(e))
        x = x0
        for k in ti.static(range(3)):
            x[k] += ((1 - a) / 2 + a * J[k]) * dx 
        return x
    ...
    def xq_2d(e, I):
        a = 0.5773502692
        x0 = QuadFEM.xc(QuadFEM.boundary_nodes(e, 0))
        xi = QuadFEM.xc(QuadFEM.boundary_nodes(e, I))
    ...
    def boundary_kernel(b: ti.types.ndarray()):
        for e in range(n_boundary_elements):
            for iq in range(4):
                x = QuadFEM.xq_2d(e, iq)
                for _i in range(4):
                    i = QuadFEM.boundary_nodes(e, _i)
                    if i < (n_yz + 1) ** 2:
                        continue
                    bi, dbidx = QuadFEM.bf_trilinear(i, x)
